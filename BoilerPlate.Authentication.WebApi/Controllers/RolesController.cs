...
public class RolesController : ControllerBase
{
    ...
    public async Task<ActionResult<RoleDto>> GetRoleByName(string name, CancellationToken cancellationToken)
    {
        ...
        if (role == null)
        {
            return NotFound(new { error = "Role not found", roleName = name });
        }
    ...
    public async Task<ActionResult<RoleDto>> CreateRole([FromBody] CreateRoleRequest request, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Role created: {RoleId} - {RoleName} in tenant {TenantId}", role.Id, role.Name, tenantId);
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    public async Task<ActionResult<RoleDto>> UpdateRole(Guid id, [FromBody] UpdateRoleRequest request, CancellationToken cancellationToken)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
        ...
        {
            return BadRequest(new { error = "System roles (Service Administrator, Tenant Administrator, User Administrator) cannot be modified", roleName = existingRole.Name });
        }

        // Also check if the new name would be a protected role
        if (IsProtectedSystemRole(request.Name) && request.Name != existingRole.Name)
        {
            return BadRequest(new { error = "Cannot rename a role to a protected system role name", roleName = request.Name });
        }
        ...
        if (role == null)
        {
            return BadRequest(new { error = "Update failed. Role name may already exist in this tenant.", roleId = id });
        }

        _logger.LogInformation("Role updated: {RoleId} - {RoleName} in tenant {TenantId}", role.Id, role.Name, tenantId);
    ...
    public async Task<IActionResult> DeleteRole(Guid id, CancellationToken cancellationToken)
    {
        ...
        {
            return BadRequest(new { error = "System roles (Service Administrator, Tenant Administrator, User Administrator) cannot be deleted", roleName = role.Name });
        }
        ...
        if (!result)
        {
            return BadRequest(new { error = "Failed to delete role", roleId = id });
        }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    public async Task<ActionResult<IEnumerable<UserDto>>> GetUsersInRole(string name, CancellationToken cancellationToken)
    {
        var tenantId = GetCurrentTenantId();
