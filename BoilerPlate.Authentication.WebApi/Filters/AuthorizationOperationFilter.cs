...
public class AuthorizationOperationFilter : IOperationFilter
{
    ...
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        var allowAnonymousAttributes = context.MethodInfo
            ...
            .Union(context.MethodInfo.DeclaringType?.GetCustomAttributes(true).OfType<AllowAnonymousAttribute>() ??
                   Enumerable.Empty<AllowAnonymousAttribute>())
            .ToList();

        // If there's an AllowAnonymous attribute, don't add security requirements
        if (allowAnonymousAttributes.Any()) return;
        ...
        if (authAttributes.Any())
        {
            ...
            if (roles.Any()) permissionDescription.Add($"Required Roles: {string.Join(", ", roles)}");
            if (policies.Any())
            {
