...
public class MessageFailureHandlerTests
{
    ...
    /// <summary>
    ///     Tests that ProcessWithFailureHandlingAsync increments the failure count when the handler fails but hasn't exceeded
    ///     the maximum.
    ///     Verifies that:
    ...
    /// <summary>
    ///     Tests that ProcessWithFailureHandlingAsync returns false (do not destroy) when failures are below the maximum
    ///     count.
    ///     Verifies that:
    ...
    /// <summary>
    ///     Tests that ProcessWithFailureHandlingAsync returns true (destroy message) when the failure count exceeds the
    ///     maximum.
    ///     Verifies that:
    ...
    /// <summary>
    ///     Tests that ProcessWithFailureHandlingAsync invokes the permanent failure callback when the failure count exceeds
    ///     the maximum.
    ///     Verifies that:
    ...
    public async Task ProcessWithFailureHandlingAsync_WhenCallbackThrows_ShouldLogErrorAndContinue()
    {
        ...
        var callback =
            new Func<TestMessage, Exception, IDictionary<string, object>?, CancellationToken, Task>((msg, ex, metadata,
                ct) => throw new Exception("Callback error"));
    ...
    public async Task ProcessWithFailureHandlingAsync_ShouldPassMetadataToHandler()
    {
        ...
        var handler = new Func<TestMessage, IDictionary<string, object>?, CancellationToken, Task>((msg, meta, ct) =>
        {
    ...
    public async Task ProcessWithFailureHandlingAsync_ShouldPassMetadataToCallback()
    {
        ...
        IDictionary<string, object>? capturedMetadata = null;
        var callback =
            new Func<TestMessage, Exception, IDictionary<string, object>?, CancellationToken, Task>((msg, ex, meta,
                ct) =>
            {
                capturedMetadata = meta;
                return Task.CompletedTask;
    ...
    /// <summary>
    ///     Tests that ProcessWithFailureHandlingAsync treats OperationCanceledException as a failure when cancellation is
    ///     requested.
    ///     Verifies that:
    ...
    public async Task ProcessWithFailureHandlingAsync_WhenCancellationRequested_ShouldHandleAsFailure()
    {
        ...
        var handler =
            new Func<TestMessage, IDictionary<string, object>?, CancellationToken, Task>(async (msg, meta, ct) =>
            {
