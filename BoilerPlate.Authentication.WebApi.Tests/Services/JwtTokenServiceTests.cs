...
public class JwtTokenServiceTests
{
    ...
    /// Scenario: A valid ApplicationUser with ID, username, email, and tenant ID, along with a list of roles, is passed to GenerateToken. The service should generate a JWT token string that can be successfully decoded and parsed by a JwtSecurityTokenHandler, confirming the token is well-formed and valid.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A user with ID, username, email, and tenant ID is passed to GenerateToken. After decoding the token, it should contain claims for the subject (sub), email, unique name (username), tenant_id, and user_id, ensuring all user information is properly embedded in the token for authorization and identification purposes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test case: GenerateToken should include role claims both as individual Role claims and as a JSON array in the roles claim.
    /// Scenario: A user is passed to GenerateToken with multiple roles (Admin, User, Guest). After decoding the token, it should contain individual ClaimTypes.Role claims for each role and a "roles" claim containing a JSON-serialized array of all roles, supporting both role-based authorization checks and bulk role retrieval scenarios.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A user is passed to GenerateToken with an empty array of roles. After decoding the token, it should not contain any ClaimTypes.Role claims or a "roles" claim, confirming that the service correctly handles users without assigned roles.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A user is passed to GenerateToken with a null roles parameter. After decoding the token, it should not contain any ClaimTypes.Role claims or a "roles" claim, confirming that the service gracefully handles null role collections without throwing exceptions.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A user with FirstName ("John") and LastName ("Doe") is passed to GenerateToken. After decoding the token, it should contain ClaimTypes.GivenName with "John" and ClaimTypes.Surname with "Doe", enabling personalized token content and user identification from claims.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test case: GenerateToken should not include GivenName or Surname claims when first name and last name are not provided.
    /// Scenario: A user without FirstName or LastName values (null or empty) is passed to GenerateToken. After decoding the token, it should not contain ClaimTypes.GivenName or ClaimTypes.Surname claims, confirming that optional user information is only included when available.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test case: GenerateToken should include an expiration time (exp claim) based on the configured ExpirationMinutes setting.
    /// Scenario: A user is passed to GenerateToken with JwtSettings configured with a specific ExpirationMinutes value. After decoding the token, it should contain a JwtRegisteredClaimNames.Exp claim and the ValidTo property should be approximately ExpirationMinutes minutes from the current time, ensuring tokens expire as configured for security purposes.
    /// </summary>
    [Fact]
    public void GenerateToken_ShouldIncludeExpiration()
    {
        ...
        jsonToken.ValidTo.Should().BeCloseTo(DateTime.UtcNow.AddMinutes(_jwtSettings.ExpirationMinutes), TimeSpan.FromSeconds(5));
    ...
    /// Scenario: A user is passed to GenerateToken with JwtSettings configured with specific Issuer and Audience values. After decoding the token, it should contain these values in the Issuer property and Audiences collection, enabling proper token validation and ensuring tokens are only accepted by intended recipients.
    /// </summary>
    [Fact]
    ...
    /// Scenario: GenerateToken is called twice with the same user. After decoding both tokens, each should contain a unique JwtRegisteredClaimNames.Jti claim value, ensuring token uniqueness and supporting token revocation and tracking scenarios where individual tokens need to be identified.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A user with a null Email property (inherited from IdentityUser) is passed to GenerateToken. After decoding the token, it should contain a JwtRegisteredClaimNames.Email claim with an empty string value, ensuring the email claim is always present even when the user has no email address configured.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A user is passed to GenerateToken with a roles collection containing valid role names ("Admin", "User") mixed with whitespace (" ") and empty strings (""). After decoding the token, it should only contain ClaimTypes.Role claims for the valid role names, confirming that invalid role values are properly filtered to maintain token integrity.
    /// </summary>
    [Fact]
    ...
    /// Scenario: GenerateRefreshToken is called. It should return a non-empty string that matches the Base64 character pattern (A-Z, a-z, 0-9, +, /, =), confirming that the refresh token is properly encoded as Base64 for safe transmission and storage.
    /// </summary>
    [Fact]
    ...
    /// Scenario: GenerateRefreshToken is called three times in succession. Each call should return a different refresh token string, ensuring cryptographic uniqueness and preventing token collisions that could lead to security vulnerabilities.
    /// </summary>
    [Fact]
    ...
    /// Scenario: GenerateRefreshToken is called and the returned string is decoded from Base64. The decoded byte array should have a length of exactly 64 bytes (512 bits), confirming that the refresh token has sufficient entropy for security purposes and matches the expected cryptographic size.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test case: GetPublicKey should return valid RSA parameters containing only public key components (modulus and exponent).
    /// Scenario: GetPublicKey is called on a JwtTokenService instance initialized with RSA keys. The returned RSAParameters should contain valid Modulus and Exponent values, but should not contain private key components (D, P, Q, DP, DQ, InverseQ), ensuring that only public key information is exposed for token validation purposes.
    /// </summary>
    [Fact]
    ...
    /// Scenario: GetPrivateKey is called on a JwtTokenService instance initialized with RSA keys. The returned RSAParameters should contain valid Modulus, Exponent, and private key components (D, P, Q), confirming that the full key pair is accessible for token signing operations.
    /// </summary>
    [Fact]
    ...
    /// Scenario: ExportPublicKeyPem is called on a JwtTokenService instance. It should return a non-empty string containing "BEGIN", "PUBLIC KEY", and "END" markers in the standard PEM format, enabling the public key to be shared with clients for token validation purposes.
    /// </summary>
    [Fact]
    ...
    /// Scenario: ExportPrivateKeyPem is called on a JwtTokenService instance. It should return a non-empty string containing "BEGIN" and "END" markers in the standard PEM format, enabling the private key to be exported for backup or configuration purposes (note: private keys should be kept secure).
    /// </summary>
    [Fact]
    ...
    /// Scenario: A JwtTokenService instance is created with JwtSettings containing a valid RSA private key in PEM format. The constructor should complete without throwing an exception, confirming that valid private keys can be imported and used for token signing.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A JwtTokenService instance is created with JwtSettings containing a valid RSA public key in PEM format. The constructor should complete without throwing an exception, confirming that valid public keys can be imported for token validation scenarios (note: token signing would not be possible with only a public key).
    /// </summary>
    [Fact]
    ...
    /// Scenario: A JwtTokenService instance is created with JwtSettings that do not contain PrivateKey or PublicKey values. The constructor should complete successfully and generate new RSA keys automatically, allowing the service to function in development environments where keys may not be pre-configured.
    /// </summary>
    [Fact]
    ...
    /// Scenario: A JwtTokenService instance is created with JwtSettings containing an invalid private key string (not in valid PEM format). The constructor should throw an Exception (typically CryptographicException), confirming that invalid key formats are properly rejected to prevent runtime errors during token operations.
    /// </summary>
    [Fact]
