...
public class OAuthRefreshTokenRequestTests
{
    ...
    /// Scenario: An OAuthRefreshTokenRequest instance is created with required properties set to test values. The GrantType should be "refresh_token" and RefreshToken should be set to a valid token value.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test case: OAuthRefreshTokenRequest should allow empty string for RefreshToken property (validation should occur at controller level).
    /// Scenario: An OAuthRefreshTokenRequest instance is created with an empty string for RefreshToken. The property should accept empty strings, though validation should reject it at the API controller level, ensuring the model doesn't enforce business rules.
    /// </summary>
    [Fact]
