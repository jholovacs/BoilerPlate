...
public class OAuthTokenRequestTests
{
    ...
    /// Scenario: An OAuthTokenRequest instance is created with all properties set to test values (GrantType, Username, Password, TenantId, Scope). All properties should be accessible and retain their assigned values, confirming the request model supports the Resource Owner Password Credentials grant type.
    /// </summary>
    [Fact]
    ...
    /// Scenario: An OAuthTokenRequest instance is created with a null Scope value. The Scope property should accept null values as it is optional in the OAuth2 specification, allowing token requests without specifying scopes.
    /// </summary>
    [Fact]
    ...
    /// Scenario: An OAuthTokenRequest instance is created without explicitly setting the GrantType property. The GrantType should default to "password" as defined in the class, supporting the Resource Owner Password Credentials grant flow by default.
    /// </summary>
    [Fact]
